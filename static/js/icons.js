/**
 * Iconos de [Lucide](https://lucide.dev) (licencia ISC/MIT), empotrados.
 *
 * Se copian los trazos en vez de cargar la librería desde un CDN: son 20 iconos
 * de los 1.500 que trae, y así la app no depende de que unpkg esté vivo ni suma
 * 70 KB de JavaScript en una conexión móvil. Mismos trazos, cero dependencias.
 *
 * Uso: `IC.plane` devuelve el SVG listo; `icon("plane", "ic-lg")` para otro tamaño.
 * En el HTML, `<span data-icon="plane"></span>` lo rellena `main.js`.
 */
...
const TRAZOS = {
  ...
  // equipaje: mochila bajo el asiento, maleta de cabina, bulto de bodega
  personal:
  ...
  any: '<path d="M12.586 2.586A2 2 0 0 0 11.172 2H4a2 2 0 0 0-2 2v7.172a2 2 0 0 0 .586 1.414l8.704 8.704a2.426 2.426 0 0 0 3.42 0l6.58-6.58a2.426 2.426 0 0 0 0-3.42z"/><circle cx="7.5" cy="7.5" r=".5" fill="currentColor"/>',
...
/** El SVG de un icono. `clase` permite otro tamaño: "ic-lg", "ic-sm". */
export function icon(nombre, clase = "") {
...
/** Atajo para interpolar: `${IC.plane}`. */
export const IC = new Proxy({}, { get: (_, nombre) => icon(String(nombre)) });

/** Rellena los `<span data-icon="…">` del HTML estático. */
export function pintarIconos(raiz = document) {
  raiz.querySelectorAll("[data-icon]").forEach((el) => {
    el.innerHTML = icon(el.dataset.icon, el.dataset.iconSize || "");
  });
}
